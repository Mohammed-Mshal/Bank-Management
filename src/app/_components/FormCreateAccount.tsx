...
export default function FormCreateAccount() {
  ...
  const { createAccount, error, loading } = useCreateAccount();
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        e.preventDefault();
        await createAccount(accountType, accountPassword, accountDescription);
      }}
    >
      ...
      <button
        ...
        type="submit"
        className={` ${
          loading ? "bg-gray-400" : "bg-indigo-800"
        } w-full py-2 text-center rounded-xl  hover:shadow-md transition-all hover:shadow-indigo-600`}
      >
